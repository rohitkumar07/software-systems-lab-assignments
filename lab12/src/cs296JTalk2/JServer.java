...
public class JServer extends Thread{
	...
	JChatComm mycomm;
	public JServer thisServer = this;
	boolean toRun; 
	JFrame chatBox;
	
	public JServer(Socket mysocket, ObjectInputStream inputStream, ObjectOutputStream outputStream){
	...
	public void run() {
		final activeServerList a = new activeServerList();
...
//			inputStream = new ObjectInputStream(mySocket.getInputStream());
		...
		while(true) {
			...
			else if (inpacket.messageString.equals("End Chat.")) {
				try {
					inputStream.close();
					outputStream.close();
					chatBox.setVisible(false);
